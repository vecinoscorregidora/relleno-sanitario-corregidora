  ...
  <div class="max-w-6xl mx-auto px-5 sm:px-8 md:px-12 py-16">
      ...
      <div class="border border-gray-200 dark:border-gray-800 p-6 rounded-xl bg-gray-50/50 dark:bg-gray-900/30">
        ...
        <ul class="space-y-2">
          <li v-for="item in modified" :key="item" class="text-sm pb-2 border-b border-gray-100 dark:border-gray-800 last:border-0 flex items-start gap-2"
              :class="isDanger(item) ? 'text-orange-700 dark:text-orange-400 font-medium' : 'text-gray-600 dark:text-gray-300'">
            <span class="w-1 h-1 rounded-full mt-2 shrink-0" :class="isDanger(item) ? 'bg-orange-500' : 'bg-gray-300 dark:bg-gray-600'"></span>
            {{ item }}
          </li>
        </ul>
      </div>
      ...
      <table class="w-full text-sm">
        ...
        <tbody>
          <tr v-for="row in shareholders" :key="row.name" class="border-b border-gray-200 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-900/30 transition-colors">
            ...
            <td class="px-4 py-3 text-right text-gray-600 dark:text-gray-300 font-mono text-xs">{{ row.shares }}</td>
            <td class="px-4 py-3 text-right text-gray-600 dark:text-gray-300 font-medium font-mono text-xs">{{ row.capital }}</td>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
      <a v-for="d in docs" :key="d.title" :href="d.href" target="_blank" rel="noopener"
         class="border border-gray-200 dark:border-gray-800 p-4 rounded-xl hover:border-blue-300 dark:hover:border-blue-700 transition-all bg-gray-50/50 dark:bg-gray-900/30 group">
        <File class="w-6 h-6 text-blue-600 dark:text-blue-400 mb-2 group-hover:scale-110 transition-transform" />
        <div class="text-sm font-medium text-gray-900 dark:text-gray-100">{{ d.title }}</div>
        <div class="text-xs text-gray-400 dark:text-gray-500 mt-0.5">{{ d.meta }}</div>
      </a>
    </div>
...
const docs = [
  { title: 'Constitución de FSO 3', meta: 'Abr 2019 | RPC', href: `${base}pdfs/01-constitucion-fso3.pdf` },
  { title: 'Modificación de Socios', meta: '4 Mar 2024', href: `${base}pdfs/02-modificacion-socios.pdf` },
  { title: 'Cambio de Objeto Social', meta: '12 Mar 2024', href: `${base}pdfs/03-cambio-objeto-social-aumento-capital.pdf` },
  { title: 'Gaceta Municipal', meta: '31 Ene 2025', href: `${base}pdfs/04-gaceta-municipal-corregidora-ene2025.pdf` },
]
</script>
